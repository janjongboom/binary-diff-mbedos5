...
off_t ufGetInt( BDFILE *lpFil ){
  ...
  if (liVal < 252)
    ...
    return 253 + bd_getc(lpFil) ;
  else if (liVal == 253) {
    liVal = bd_getc(lpFil) ;
    liVal = (liVal << 8) + bd_getc(lpFil) ;
    return liVal ;
  }
  else if (liVal == 254) {
    liVal = bd_getc(lpFil) ;
...
*******************************************************************************/
void jpatch ( BDFILE *asFilOrg, BDFILE *asFilPch, BDFILE *asFilOut )
{
  ...
  liOpr = ESC ;
  while ((liInp = bd_getc(asFilPch)) != EOF) {
	// Parse an operator: ESC liOpr [lzOff]
    if (liInp == ESC) {
      liInp = bd_getc(asFilPch);
      switch(liInp) {
        case MOD:
          liOpr = MOD;
          if (giVerbse == 1) {
        ...
        case DEL:
          ...
          if (bd_fseek(asFilOrg, lzOff + lzMod, SEEK_CUR) != 0) {
        ...
        case EQL:
          ...
          if (lzMod > 0) {
              if (bd_fseek(asFilOrg, lzMod, SEEK_CUR) != 0) {
          ...
          while (lzOff > BLKSZE) {
              if (bd_fread(&lcDta, 1, BLKSZE, asFilOrg ) != BLKSZE) {
                  fprintf(stderr, "Error reading original file.\n");
                  exit(EXI_RED);
              }
              if (bd_fwrite(&lcDta, 1, BLKSZE, asFilOut) != BLKSZE) {
          ...
          if (lzOff > 0){
              if (bd_fread(&lcDta, 1, lzOff, asFilOrg) != lzOff) {
                  fprintf(stderr, "Error reading original file.\n");
                  exit(EXI_RED);
              }
              if (bd_fwrite(&lcDta, 1, lzOff, asFilOut) != lzOff) {
                  fprintf(stderr, "Error writing output file.\n");
                  exit(EXI_WRI);
              }
          }
        ...
        case BKT:
          ...
          if (bd_fseek(asFilOrg, lzMod - lzOff, SEEK_CUR) != 0) {
